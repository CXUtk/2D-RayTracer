...
#include "Utils.h"
BoundingBox::BoundingBox(glm::vec2 minPos, glm::vec2 maxPos) : _minPos(minPos), _maxPos(maxPos) {
}
...
bool BoundingBox::rayIntersect(const Ray& ray, float& tNear) const {
    ...
    float tMin = std::max(tRange[0][0], tRange[1][0]), tMax = std::min(tRange[0][1], tRange[1][1]);
